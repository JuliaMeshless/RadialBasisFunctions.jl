...
@testset "Hermite Integration Tests" begin
    ...
    @testset "Basic Operator Construction" begin
        # Verify that operators can be constructed with various BC combinations

        @testset "Laplacian with All BC Types" begin
            data, is_boundary, _, _ = create_2d_domain()

            # Dirichlet
            @test_nowarn laplacian(
                data,
                data,
                basis_phs,
                is_boundary,
                [RBF.Dirichlet(), RBF.Dirichlet()],
            ...
            @test_nowarn laplacian(
                ...
                basis_phs,
                is_boundary,
                [RBF.Robin(1.0, 1.0), RBF.Robin(2.0, 1.0)],
                [SVector(1.0, 0.0), SVector(-1.0, 0.0)],
            )
            ...
            @test_nowarn laplacian(
                ...
                basis_phs,
                is_boundary,
                [RBF.Dirichlet(), RBF.Neumann()],
                [SVector(1.0, 0.0), SVector(-1.0, 0.0)],
            )
        end

        @testset "Gradient with BCs" begin
            data, is_boundary, bcs, normals = create_2d_domain()
        ...
        @testset "Directional Derivative with BCs" begin
            data, is_boundary, bcs, normals = create_2d_domain()
            direction = [SVector(1.0, 1.0) for _ in data]  # Diagonal direction

            op = directional(data, data, direction, basis_phs, is_boundary, bcs, normals)
            @test op isa RBF.RadialBasisOperator
        end
    end
