...
"""
function validate_gradient(ad_grad, loss, input; rtol=1.0e-4, check_nonzero=true)
    fd_grad = FD.grad(FD.central_fdm(5, 1), loss, input)[1]
    if check_nonzero
        @test !all(iszero, ad_grad)
    end
    return @test isapprox(ad_grad, fd_grad; rtol=rtol)
end
...
"""
function validate_scalar_gradient(ad_val, loss, input; rtol=1.0e-3, check_nonzero=true)
    fd_val = FD.central_fdm(5, 1)(loss, input)
    if check_nonzero
        @test !iszero(ad_val)
    end
    return @test isapprox(ad_val, fd_val; rtol=rtol)
end

"""
    make_directional_test_data(; N=25, k=10)

Returns (points, N, adjl, pts_flat, n_const, n_varying).
`n_const` is a normalized constant direction vector [1/√2, 1/√2].
`n_varying` is a vector of spatially varying normalized direction vectors.
"""
function make_directional_test_data(; N=25, k=10)
    ...
    n_const = [1.0 / sqrt(2), 1.0 / sqrt(2)]
    # Spatially varying directions (each normalized)
